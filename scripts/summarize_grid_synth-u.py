#!/usr/bin/env python3
"""Parse grid search logs for PTFG V2 on synth-u and rank combos."""
import json
import re
import sys
...
def _default_log_dir() -> Path:
    """Prefer ./log/grid_search from cwd; else repo log/grid_search."""
    cwd_grid = Path.cwd() / "log" / "grid_search"
    if cwd_grid.is_dir():
        return cwd_grid.resolve()
    return (_PROJECT_ROOT / "log" / "grid_search").resolve()
...
def main():
    logs = sorted(LOG_DIR.glob("ptfg_synth-u_*.log"))
    if not logs:
