"""MaskGIT prior for conditional TimeVQVAE sampling."""
...
from contsg.models.timevqvae_modules.generators.bidirectional_transformer import (
    BidirectionalTransformer,
)
from contsg.models.timevqvae_modules.utils import quantize
...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vqvae is None:
            vqvae = TimeVQVAE(config)
        object.__setattr__(self, "_vqvae", vqvae)

        if pretrained_model_path and pretrained_model_path != "auto":
            self._load_vqvae_weights(pretrained_model_path)
    ...
    @property
    def encoder_l(self):
        return self._vqvae.encoder_l

    @property
    def decoder_l(self):
        return self._vqvae.decoder_l

    @property
    def vq_model_l(self):
        return self._vqvae.vq_model_l

    @property
    def encoder_h(self):
        return self._vqvae.encoder_h

    @property
    def decoder_h(self):
        return self._vqvae.decoder_h

    @property
    def vq_model_h(self):
        return self._vqvae.vq_model_h

    def _strip_state_prefix(self, state_dict):
        cleaned = {}
        for key, value in state_dict.items():
            if key.startswith("vqvae."):
                new_key = key[6:]
            elif key.startswith("model.vqvae."):
                new_key = key[len("model.vqvae."):]
            else:
                new_key = key
            cleaned[new_key] = value
        return cleaned

    def _load_vqvae_weights(self, path: str) -> None:
        state = torch.load(path, map_location="cpu")
        if isinstance(state, dict) and "state_dict" in state:
            state_dict = state["state_dict"]
        else:
            state_dict = state
        state_dict = self._strip_state_prefix(state_dict)
        self._vqvae.load_state_dict(state_dict, strict=False)
