...
from .attention import Spatial1DTransformer
from .conditioning_mlp import ConditioningMLP
from .prototype import DomainUnifiedPrototyper
    ...
    def __init__(self,
        ...
        if context_dim is not None:
            assert use_spatial_transformer, (
                "context_dim is set but use_spatial_transformer is False. "
                "Set use_spatial_transformer=True to enable cross-attention conditioning."
            )
...
# NOTE: Legacy Bridge class - kept for backward compatibility
# This class is no longer actively maintained. For new code, use the
# BridgeModule in contsg.models.bridge instead.
class Bridge(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.model = instantiate_from_config(config["unet"])
        self.cond_stage_model = instantiate_from_config(config["cond_stage_model"])
        self.cond_stage_model.to(config["device"])
        self.cond_drop_prob = config["cond_drop_prob"]


    def get_learning_conditioning(self, c):
        c, mask = self.cond_stage_model(c)
        return c, mask

    def forward(self, x, tp, attr_emb, diffusion_step, x_example=None):
        c, mask = self.cond_stage_model(x_example) # c, mask shape: [B, 1, 32, 32]
